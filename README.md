...
- Ranked code + balance candidate: **PASS** for the `season-zero-r2` certified pool; post-merge Ranked Release Certification and two-player browser certification are green.
- Ranked production activation remains **fail-closed by default** (`RANKED_RELEASE_CERTIFIED=false`) until the hosting operator deliberately enables the certified release and runtime `rankedEnabled`; an open Ranked season is also required.
...
The current `season-zero-r2` audit was regenerated by the exact post-merge Ranked Release Certification artifact:
...
- **6/6 stable** matchups;
- maximum seed-stratum deviation **7.5 percentage points** (limit 15);
- matchup range **45.3%–54.8%**;
- Ranked balance gate: **PASS**.

See `BALANCE_AUDIT_2.97.json`, `docs/AUDIT_2.97.md` and `docs/RANKED-R2-RELEASE-CERTIFICATE.md`.
...
For **Ranked** activation, the deployment must additionally pass:
...
Then confirm the deployed provenance and open season before enabling runtime `rankedEnabled=true` through the step-up protected LiveOps settings surface. Ranked reports enabled only when the release flag, runtime switch and season window are all valid. The preferred kill switch is `rankedEnabled=false`.
...
See `docs/ALPHA-RELEASE-CANDIDATE-1-0.md`, `docs/PUBLIC-DEPLOYMENT-PROVENANCE-1-0.md`, `docs/RELEASE.md`, `docs/TESTING.md`, `docs/SECURITY.md`, `docs/ARCHITECTURE.md` and `docs/RANKED-R2-RELEASE-CERTIFICATE.md`.
