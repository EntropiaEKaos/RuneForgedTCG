...
const upgradePlans: Array<{ when: (versions: Set<string>) => boolean; files: string[] }> = [
  ...
  { when: (v) => v.has("2.96"), files: ["drizzle/0037_schema_replay_hotfix_2_96_1.sql", "drizzle/0038_engineering_integrity_2_96_2.sql", "drizzle/0039_ranked_certification_2_97.sql", "drizzle/0040_pvp_content_snapshot_2_97.sql", reactionMigration] },
  { when: (v) => v.has("2.94") || v.has("2.95"), files: ["drizzle/0036_sentinelas_convergence_2_96.sql", "drizzle/0037_schema_replay_hotfix_2_96_1.sql", "drizzle/0038_engineering_integrity_2_96_2.sql", "drizzle/0039_ranked_certification_2_97.sql", "drizzle/0040_pvp_content_snapshot_2_97.sql", reactionMigration] },
  { when: (v) => v.has("2.93"), files: ["drizzle/0035_release_hardening_2_94.sql", "drizzle/0036_sentinelas_convergence_2_96.sql", "drizzle/0037_schema_replay_hotfix_2_96_1.sql", "drizzle/0038_engineering_integrity_2_96_2.sql", "drizzle/0039_ranked_certification_2_97.sql", "drizzle/0040_pvp_content_snapshot_2_97.sql", reactionMigration] },
  ...
  try {
    ...
    if (versions.has("2.97")) {
      const requiredColumns = await pool.query<{ table_name: string; column_name: string }>(
        ...
        [["ranked_season_id", "content_snapshot", "content_hash", "reaction_state"], ["rules_version", "deck_pool_version"]],
      );
      const columnKeys = new Set(requiredColumns.rows.map((row) => `${row.table_name}.${row.column_name}`));
      const requiredColumnKeys = [
        "pvp_rooms.ranked_season_id", "pvp_rooms.content_snapshot", "pvp_rooms.content_hash", "pvp_rooms.reaction_state",
