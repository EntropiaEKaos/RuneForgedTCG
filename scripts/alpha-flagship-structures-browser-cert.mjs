...
const probes = [
  ["rfalpha_ember_structure_forge_bastion", "Bastião da Forja Rubra", "/art/cards/flagship/emberhold/rfalpha_ember_structure_forge_bastion.webp", "11a-structure-emberhold-art-viewer.png"],
  ["rfalpha_tide_structure_silent_beacon", "Farol das Marés Silenciosas", "/art/cards/flagship/tidecall/rfalpha_tide_structure_silent_beacon.webp", "11b-structure-tidecall-art-viewer.png"],
  ["rfalpha_wood_structure_root_circle", "Círculo das Raízes de Ferro", "/art/cards/flagship/ironwood/rfalpha_wood_structure_root_circle.webp", "11c-structure-ironwood-art-viewer.png"],
  ["rfalpha_void_structure_hollow_obelisk", "Obelisco da Sombra Oca", "/art/cards/flagship/voidborn/rfalpha_void_structure_hollow_obelisk.webp", "11d-structure-voidborn-art-viewer.png"],
  ["rfalpha_forest_structure_ancestral_den", "Toca da Matilha Ancestral", "/art/cards/flagship/florestia/rfalpha_forest_structure_ancestral_den.webp", "11e-structure-florestia-art-viewer.png"],
  ["rfalpha_storm_structure_first_thunder", "Torre do Primeiro Trovão", "/art/cards/flagship/tempestade/rfalpha_storm_structure_first_thunder.webp", "11f-structure-tempestade-art-viewer.png"],
...
async function hoverStructure(cdp, probe) {
...
async function openViewer(cdp, probe) {
  const clicked = await evaluate(cdp, `(() => {
    const trigger = document.querySelector('[data-card-art-viewer-trigger="${probe.defId}"]');
  ...
  const state = await evaluate(cdp, `(() => {
    ...
    return {
      ...
      backgroundSize: image ? getComputedStyle(image).backgroundSize : '',
...
async function closeViewer(cdp, probe) {
  const clicked = await evaluate(cdp, `(() => {
    const close = document.querySelector('[data-card-art-viewer-close="${probe.defId}"]');
...
async function main() {
  ...
  const profileDir = await mkdtemp(join(tmpdir(), "runeforge-flagship-structures-"));
  ...
  try {
    ...
    for (const probe of probes) {
      ...
      await hoverStructure(cdp, probe);
      await openViewer(cdp, probe);
      await capture(cdp, probe.screenshot);
      await closeViewer(cdp, probe);
    }

    console.log("FLAGSHIP ART BATCH B BROWSER CERT: PASS — six Structure WebP masters served, rendered and opened in VER ARTE");
  } finally {
    try { cdp?.close(); } catch {}
...
void main().catch((error) => {
  console.error("FLAGSHIP ART BATCH B BROWSER CERT: FAIL", error);
  process.exitCode = 1;
});
