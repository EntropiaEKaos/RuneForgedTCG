...
const probes = [
  ["rfalpha_ember_trap_ash_snare", "Cilada de Brasas", "/art/cards/flagship/emberhold/rfalpha_ember_trap_ash_snare.webp", "13a-trap-emberhold-art-viewer.png"],
  ["rfalpha_tide_trap_countercurrent", "Selo da Contramaré", "/art/cards/flagship/tidecall/rfalpha_tide_trap_countercurrent.webp", "13b-trap-tidecall-art-viewer.png"],
  ["rfalpha_wood_trap_emergency_bark", "Casca de Emergência", "/art/cards/flagship/ironwood/rfalpha_wood_trap_emergency_bark.webp", "13c-trap-ironwood-art-viewer.png"],
  ["rfalpha_void_trap_early_eclipse", "Eclipse Prematuro", "/art/cards/flagship/voidborn/rfalpha_void_trap_early_eclipse.webp", "13d-trap-voidborn-art-viewer.png"],
  ["rfalpha_forest_trap_pack_ambush", "Emboscada da Alcateia", "/art/cards/flagship/florestia/rfalpha_forest_trap_pack_ambush.webp", "13e-trap-florestia-art-viewer.png"],
  ["rfalpha_storm_trap_crosswind", "Contravento Súbito", "/art/cards/flagship/tempestade/rfalpha_storm_trap_crosswind.webp", "13f-trap-tempestade-art-viewer.png"],
...
async function hoverTrap(cdp, probe) {
...
async function main() {
  ...
  try {
    ...
    for (const probe of probes) {
      ...
      await hoverTrap(cdp, probe);
      await openViewer(cdp, probe);
      await capture(cdp, probe.screenshot);
      await closeViewer(cdp, probe);
    }

    console.log("FLAGSHIP ART BATCH D BROWSER CERT: PASS — six Trap WebP masters served, rendered and opened in VER ARTE");
  } finally {
    try { cdp?.close(); } catch {}
...
void main().catch((error) => {
  console.error("FLAGSHIP ART BATCH D BROWSER CERT: FAIL", error);
  process.exitCode = 1;
});
