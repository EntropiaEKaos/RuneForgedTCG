...
const baselineS07 = structuredClone(CARDS.van_forest_s07);
const baselineS08 = structuredClone(CARDS.van_forest_s08);
if (!baselineS07?.spell || baselineS07.spell.kind !== "draw" || baselineS07.spell.amount !== 2) throw new Error("s07 draw-2 contract missing");
if (!baselineS08?.spell || baselineS08.spell.kind !== "buffRace") throw new Error("s08 tribal contract missing");

const variants = [
  { id: "baseline", s07Cost: 5, s08Dual: false },
  { id: "s07_cost4", s07Cost: 4, s08Dual: false },
  { id: "s07_cost3", s07Cost: 3, s08Dual: false },
  { id: "s08_dual", s07Cost: 5, s08Dual: true },
  { id: "s07_cost4_s08_dual", s07Cost: 4, s08Dual: true },
  { id: "s07_cost3_s08_dual", s07Cost: 3, s08Dual: true },
] as const;

function round1(value: number): number { return Math.round(value * 10) / 10; }
function resetCards() {
  CARDS.van_forest_s07 = structuredClone(baselineS07);
  CARDS.van_forest_s08 = structuredClone(baselineS08);
}
function applyVariant(variant: typeof variants[number]) {
  resetCards();
  CARDS.van_forest_s07.cost = variant.s07Cost;
  if (variant.s08Dual) {
...
function summarizeTarget(targetId: string, rows: MatchRow[]) {
  ...
  return { targetId, games: wins + losses + draws, wins, losses, draws, winRate: round1(wins / Math.max(1, wins + losses) * 100), matchupHealth: { healthy, watch, critical }, matchups: matchups.sort((a, b) => a.winRate - b.winRate || a.opponent.localeCompare(b.opponent)) };
}
...
const comparison = variantReports.map((variant) => ({
  ...
  targets: variant.targets.map((target) => {
    ...
    return { targetId: target.targetId, winRate: target.winRate, deltaVsBaseline: round1(target.winRate - base.winRate), matchupHealth: target.matchupHealth, criticalDelta: target.matchupHealth.critical - base.matchupHealth.critical };
  }),
}));
const qualityGate = poolErrors.length === 0 && incompleteStrata === 0 ? "pass" : "blocked";
const report = {
  version: "Vanilla 1.8 Florestia card-advantage screen",
  methodology: "Exact production Balance Lab with common deterministic seeds. Screens the minimum s07 cost corrections justified by same-cycle Vanilla draw-2 benchmarking, alone and with the separately proven s08 dual-race semantic correction. No recipe, AI or engine mutation.",
...
if (qualityGate !== "pass") {
  console.error(`VANILLA 1.8 FLORESTIA CARD-ADVANTAGE SCREEN: BLOCKED — ${poolErrors.length} pool errors · ${incompleteStrata} incomplete strata`);
  process.exitCode = 1;
} else console.log(`VANILLA 1.8 FLORESTIA CARD-ADVANTAGE SCREEN: PASS — ${report.totalGames} games · ${variants.length} paired variants`);
