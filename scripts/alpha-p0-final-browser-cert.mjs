...
const subject = {
  defId: "wood_cub",
  cardName: "Thornback Cub",
  artPath: "/art/cards/alpha-p0/ironwood/wood_cub.webp",
};
const activeP0Paths = [
  ...
  "/art/cards/alpha-p0/emberhold/ember_sprinter.webp",
  "/art/cards/alpha-p0/ironwood/wood_ward.webp",
  "/art/cards/alpha-p0/emberhold/ember_face.webp",
  "/art/cards/alpha-p0/emberhold/ember_drake.webp",
  "/art/cards/alpha-p0/emberhold/ember_stun.webp",
  "/art/cards/alpha-p0/florestia/forest_canopy_warden.webp",
  "/art/cards/alpha-p0/florestia/forest_cub.webp",
  "/art/cards/alpha-p0/tempestade/storm_dashbolt.webp",
  "/art/cards/alpha-p0/tempestade/storm_eye.webp",
  "/art/cards/alpha-p0/tempestade/storm_herald.webp",
  "/art/cards/alpha-p0/tempestade/storm_lightning.webp",
  "/art/cards/alpha-p0/tempestade/storm_sky_sentinel.webp",
  "/art/cards/alpha-p0/tempestade/storm_strikecaller.webp",
  "/art/cards/alpha-p0/tidecall/tide_sprite.webp",
  "/art/cards/alpha-p0/voidborn/void_drain.webp",
  "/art/cards/alpha-p0/ironwood/wood_cub.webp",
];
...
async function openSubjectViewer(cdp) {
  ...
  await waitUntil(() => evaluate(cdp, `Boolean(document.querySelector('[data-card-art-viewer-trigger="${subject.defId}"]'))`), "final P0 VER ARTE trigger");
  ...
  assert.equal(clicked, true, "Final P0 VER ARTE button could not be clicked");
  await waitUntil(() => evaluate(cdp, `Boolean(document.querySelector('[data-card-art-viewer="${subject.defId}"]'))`), "final P0 full-art dialog");
  ...
  assert.equal(state.role, "dialog", "Final P0 full-art viewer must expose dialog semantics");
  assert.equal(state.modal, "true", "Final P0 full-art viewer must be modal");
  assert.match(state.label, /Thornback Cub/i, "Final P0 full-art viewer must identify Thornback Cub");
  assert.ok(state.background.includes(subject.artPath), `Final P0 full-art viewer must render ${subject.artPath}`);
  assert.equal(state.overflow, "hidden", "Final P0 full-art viewer must lock background scrolling");
}
...
async function capture(cdp) {
  ...
  assert.ok(metrics.scrollWidth <= metrics.innerWidth + 2, "Final P0 art viewer has horizontal overflow");
  const screenshot = await cdp.call("Page.captureScreenshot", { format: "png", fromSurface: true, captureBeyondViewport: false });
  await writeFile(join(outputDir, "52-alpha-p0-wood-cub-art-viewer.png"), Buffer.from(screenshot.data, "base64"));
}
...
async function main() {
  ...
  assert.equal(activeP0Paths.length, 21, "Final P0 browser certification must cover all 21 active masters");
  for (const path of activeP0Paths) await assertServedWebp(path);

  const profileDir = await mkdtemp(join(tmpdir(), "runeforge-p0-final-viewer-"));
  ...
  try {
    ...
    console.log("ALPHA P0 FINAL BROWSER CERT: PASS — 21 active P0 WebPs served + Thornback Cub runtime viewer certified");
  } finally {
    try { cdp?.close(); } catch {}
...
void main().catch((error) => {
  console.error("ALPHA P0 FINAL BROWSER CERT: FAIL", error);
  process.exitCode = 1;
});
