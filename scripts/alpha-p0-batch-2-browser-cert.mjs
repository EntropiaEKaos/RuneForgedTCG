...
const subject = {
  ...
  cardName: "Blitzrunner",
  artPath: "/art/cards/alpha-p0/emberhold/ember_sprinter.webp",
};
const activeP0Paths = [
...
async function dismissRecoveryHandoffIfPresent(cdp) {
  ...
  if (handoff?.present) assert.equal(handoff.dismissed, true, "Recovery-key handoff could not be dismissed before Batch 2 art certification");
}
...
async function openSubjectViewer(cdp) {
  ...
  await waitUntil(() => evaluate(cdp, `Boolean(document.querySelector('[data-card-art-viewer-trigger="${subject.defId}"]'))`), "Batch 2 VER ARTE trigger");
  ...
  assert.equal(clicked, true, "Batch 2 VER ARTE button could not be clicked");
  await waitUntil(() => evaluate(cdp, `Boolean(document.querySelector('[data-card-art-viewer="${subject.defId}"]'))`), "Batch 2 full-art dialog");
  ...
  assert.equal(state.role, "dialog", "Batch 2 full-art viewer must expose dialog semantics");
  assert.equal(state.modal, "true", "Batch 2 full-art viewer must be modal");
  assert.match(state.label, /Blitzrunner/i, "Batch 2 full-art viewer must identify Blitzrunner");
  assert.ok(state.background.includes(subject.artPath), `Batch 2 full-art viewer must render ${subject.artPath}`);
  assert.equal(state.overflow, "hidden", "Batch 2 full-art viewer must lock background scrolling");
}
...
async function capture(cdp) {
  ...
  assert.ok(metrics.scrollWidth <= metrics.innerWidth + 2, "Batch 2 art viewer has horizontal overflow");
  const screenshot = await cdp.call("Page.captureScreenshot", { format: "png", fromSurface: true, captureBeyondViewport: false });
  await writeFile(join(outputDir, "47-alpha-p0-ember-sprinter-art-viewer.png"), Buffer.from(screenshot.data, "base64"));
}
...
async function main() {
  ...
  const profileDir = await mkdtemp(join(tmpdir(), "runeforge-p0-batch-2-viewer-"));
  ...
  try {
    ...
    console.log("ALPHA P0 BATCH 2 BROWSER CERT: PASS — 10 active P0 WebPs served + Blitzrunner runtime viewer certified");
  } finally {
    try { cdp?.close(); } catch {}
...
void main().catch((error) => {
  console.error("ALPHA P0 BATCH 2 BROWSER CERT: FAIL", error);
  process.exitCode = 1;
});
