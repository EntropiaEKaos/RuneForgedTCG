...
const probes = [
  ["rfalpha_ember_ritual_red_rite", "Rito da Chama Soberana", "/art/cards/flagship/emberhold/rfalpha_ember_ritual_red_rite.webp", "12a-ritual-emberhold-art-viewer.png"],
  ["rfalpha_tide_ritual_memory_tide", "Liturgia da Maré da Memória", "/art/cards/flagship/tidecall/rfalpha_tide_ritual_memory_tide.webp", "12b-ritual-tidecall-art-viewer.png"],
  ["rfalpha_wood_ritual_ancient_roots", "Rito das Raízes Antigas", "/art/cards/flagship/ironwood/rfalpha_wood_ritual_ancient_roots.webp", "12c-ritual-ironwood-art-viewer.png"],
  ["rfalpha_void_ritual_emptiness", "Liturgia da Ausência", "/art/cards/flagship/voidborn/rfalpha_void_ritual_emptiness.webp", "12d-ritual-voidborn-art-viewer.png"],
  ["rfalpha_forest_ritual_green_moon", "Uivo da Lua Verde", "/art/cards/flagship/florestia/rfalpha_forest_ritual_green_moon.webp", "12e-ritual-florestia-art-viewer.png"],
  ["rfalpha_storm_ritual_eye_of_storm", "Rito do Olho da Tempestade", "/art/cards/flagship/tempestade/rfalpha_storm_ritual_eye_of_storm.webp", "12f-ritual-tempestade-art-viewer.png"],
...
async function hoverRitual(cdp, probe) {
...
async function main() {
  ...
  const profileDir = await mkdtemp(join(tmpdir(), "runeforge-flagship-rituals-"));
  ...
  try {
    ...
    for (const probe of probes) {
      ...
      await hoverRitual(cdp, probe);
      await openViewer(cdp, probe);
      await capture(cdp, probe.screenshot);
      await closeViewer(cdp, probe);
    }

    console.log("FLAGSHIP ART BATCH C BROWSER CERT: PASS — six Mana Ritual WebP masters served, rendered and opened in VER ARTE");
  } finally {
    try { cdp?.close(); } catch {}
...
void main().catch((error) => {
  console.error("FLAGSHIP ART BATCH C BROWSER CERT: FAIL", error);
  process.exitCode = 1;
});
