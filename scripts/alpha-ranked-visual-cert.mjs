...
async function clickTextIfPresent(cdp, text) {
  const encoded = JSON.stringify(text);
  return evaluate(cdp, `(() => { const target=[...document.querySelectorAll('button,a,[role="button"]')].find((el) => !el.disabled && (el.textContent || '').replace(/\\s+/g,' ').trim().includes(${encoded})); if (!target) return false; target.click(); return true; })()`);
}
...
async function main() {
  ...
  try {
    ...
    await waitUntil(() => evaluate(cdp, `document.body?.innerText?.includes('SALVE SUA CHAVE DE RECUPERAÇÃO') === true || document.body?.innerText?.includes('Gate competitivo fechado') === true`), "Ranked session bootstrap");
    if (await clickTextIfPresent(cdp, "JÁ GUARDEI")) await sleep(250);
    await waitUntil(() => evaluate(cdp, `document.body?.innerText?.includes('Gate competitivo fechado') === true`), "Ranked fail-closed state");
    ...
    assert.match(top.bodyText, /Bloqueado/);
    ...
    console.log("ALPHA RANKED VISUAL CERT: PASS — fail-closed hero + progression screenshots captured");
  } finally {
    try { cdp?.close(); } catch {}
