...
for (let left = 0; left < DECKS.length; left += 1) {
  for (let right = left + 1; right < DECKS.length; right += 1) {
    rows.push(runBalanceSimulation(DECKS[left].id, DECKS[right].id, games, 288_000 + left * 101 + right));
  }
}

const health = summarizeBalance(rows);
console.log(JSON.stringify({ version: "2.92", gamesPerMatchup: games, matchups: rows.length, health }, null, 2));
