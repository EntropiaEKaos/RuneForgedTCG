...
const report = {
  version: "1.11-florestia-diagnostic-1",
  methodology:
    "Read-only Florestia Ascendant utilization diagnostic using the certified Vanilla Balance Lab simulator, exact five deterministic strata and current product recipes from the post-1.10 baseline. No CardDef, recipe, AI or rules mutation.",
...
if (qualityGate !== "pass") {
  console.error(`VANILLA 1.11 FLORESTIA DIAGNOSTIC: BLOCKED — ${poolErrors.length} pool errors · ${telemetryErrors.length} telemetry errors · ${incompleteStrata} incomplete strata`);
  process.exitCode = 1;
} else {
  console.log(
    `VANILLA 1.11 FLORESTIA DIAGNOSTIC: PASS — ${report.simulation.totalGames} games · ${matchups.length}/11 matchups · WR=${report.target.winRate}% · ${healthy}H/${watch}W/${critical}C`,
  );
}
