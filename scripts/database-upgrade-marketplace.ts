...
async function main() {
  ...
  try {
    ...
    try {
      ...
      for (const file of ["drizzle/0043_p2p_marketplace.sql", "drizzle/0044_card_cosmetics.sql", "drizzle/0045_identity_auth.sql", "drizzle/0046_admin_fx_presets.sql", "drizzle/0047_admin_fx_associations.sql", "drizzle/0048_deck_collectible_appearances.sql"]) {
        const sql = await fs.readFile(path.join(process.cwd(), file), "utf8");
        await client.query(sql);
      }
      await client.query("commit");
      console.log("DATABASE UPGRADE — P2P MARKETPLACE + CARD COSMETICS + IDENTITY AUTH + FX PRESETS + FX ASSOCIATIONS + DECK COLLECTIBLES: PASS");
