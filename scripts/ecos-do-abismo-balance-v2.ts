...
async function main(): Promise<void> {
  ...
  function removeOne(defId: string): void {
    ...
    if (index < 0) throw new Error(`Recipe v8 expected ${defId} in Ecos do Abismo v1.`);
  ...
  if (cards.length !== 40) throw new Error(`Recipe v8 must remain exactly 40 cards; got ${cards.length}.`);
  const validation = validateDeck(cards);
  if (!validation.ok) throw new Error(`Recipe v8 is illegal: ${validation.errors.join(" | ")}`);
  if (validation.regions.length !== 2 || !validation.regions.includes("Tidecall") || !validation.regions.includes("Voidborn")) {
    throw new Error(`Recipe v8 must remain Tidecall/Voidborn; got ${validation.regions.join(", ")}.`);
  }

  deck.cards.splice(0, deck.cards.length, ...cards);
  console.log("ECOS RECIPE V8 CANDIDATE: v6 -1 Hollow Rift Colossus · +1 Dead Tide Devourer");
