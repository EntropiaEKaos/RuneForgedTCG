...
async function main(): Promise<void> {
  ...
  function removeOne(defId: string): void {
    ...
    if (index < 0) throw new Error(`Recipe v15 expected ${defId} in Ecos do Abismo v1.`);
  ...
  if (!validation.ok) throw new Error(`Recipe v15 is illegal: ${validation.errors.join(" | ")}`);
  if (validation.regions.length !== 2 || !validation.regions.includes("Tidecall") || !validation.regions.includes("Voidborn")) {
    throw new Error(`Recipe v15 must remain Tidecall/Voidborn; got ${validation.regions.join(", ")}.`);
  }

  deck.cards.splice(0, deck.cards.length, ...cards);
  console.log("ECOS RECIPE V15 CANDIDATE: v6 recipe · Hollow Rift Colossus simulated without Overwhelm");
