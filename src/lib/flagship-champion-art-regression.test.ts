...
assert.ok(structureWorkflow.includes("npm run build"), "dedicated visual workflow must certify production build output");
...
for (const source of [championGenerator, structureGenerator, structureBrowser, structureWorkflow]) {
  for (const forbidden of [
    ...
    assert.equal(source.includes(forbidden), false, `Flagship Batch B tooling must not mutate frozen surface ${forbidden}`);
  }
}
