...
import { sanitizeArchetypeDefinition, sanitizeCompositeEffectDefinition, sanitizeKeywordBehavior } from "@/game/mechanics-authoring";
import { ENGINE_VERSION, RULESET_VERSION } from "@/game/version";
import { validateLiveOps } from "@/lib/live-ops-rules";

export const CONTENT_RESOURCES = ["cards", "keywords", "effects", "archetypes", "races", "classes", "interactions", "collections", "card-meta", "events", "promotions", "fx-presets"] as const;
export type ContentResource = (typeof CONTENT_RESOURCES)[number];

export function isContentResource(resource: string): resource is ContentResource { return (CONTENT_RESOURCES as readonly string[]).includes(resource); }
...
export function validateContent(resource: string, row: any) {
  ...
  if (resource === "cards") {
    if (!row?.data || typeof row.data !== "object") errors.push("Card data is missing.");
      ...
      if (!data.name || !data.defId || !data.type || !data.region || !data.rarity) errors.push("Card identity is incomplete.");
      if (data.cost !== undefined && (Number(data.cost) < 0 || Number(data.cost) > 20)) errors.push("Card cost must be between 0 and 20.");
  ...
  if (resource !== "card-meta" && (!stableKey || !/^[a-z0-9][a-z0-9_-]{1,63}$/.test(String(stableKey)))) errors.push("Stable key must match /^[a-z0-9][a-z0-9_-]{1,63}$/.");
  ...
  if (resource === "fx-presets") {
    if (!["motion", "timeline", "gpu"].includes(String(row?.renderer || ""))) errors.push("FX renderer must be motion, timeline, or gpu.");
    if (!["subtle", "standard", "cinematic"].includes(String(row?.intensity || ""))) errors.push("FX intensity must be subtle, standard, or cinematic.");
  ...
  if (["events", "promotions"].includes(resource) && row?.status === "published" && !row?.startsAt) warnings.push("Published live content has no start time.");
  if (resource === "collections" && row?.status === "published" && !row?.code) errors.push("Published collection requires a code.");
  const liveOps = validateLiveOps(resource, row);
  errors.push(...liveOps.errors); warnings.push(...liveOps.warnings);
...
export function requiredApprovalStages(resource: string): ApprovalStage[] {
  if (["cards", "card-meta", "keywords", "effects", "archetypes", "races", "classes", "interactions", "collections", "fx-presets"].includes(resource)) return ["content", "qa"];
  if (resource === "events" || resource === "promotions") return ["content", "qa", "liveops"];
  return ["content", "qa"];
}
export function isValidApprovalStage(stage: string): stage is ApprovalStage { return (APPROVAL_STAGES as readonly string[]).includes(stage); }
