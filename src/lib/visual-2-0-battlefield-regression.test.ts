...
const viewportPolish = readFileSync("src/app/styles/visual-2-0-viewport-polish.css", "utf8");
...
assert.ok(layout.includes('import "./styles/alpha-battlefield.css";\nimport "./styles/visual-2-0-battlefield.css";\nimport "./styles/visual-2-0-battlefield-polish.css";\nimport "./styles/visual-2-0-viewport-polish.css";'), "Visual 2.0 screenshot-driven layers must load after the certified Alpha battlefield layer");
...
assert.ok(viewportPolish.includes("min-height: 50px") && viewportPolish.includes("min-height: 36px"), "Certified desktop viewport must retain the primary action surface below the complete hand");
