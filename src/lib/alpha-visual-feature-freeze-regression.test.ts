 ...
 *
 * Pack opening premium break-glass 2026-09-14: layout.tsx is intentionally
 * recertified only to load Visual 5.7 after Visual 5.6. Visual 5.7 styles the
 * existing Store reveal dialog and its already-rendered rarity/cosmetic surfaces.
 * StoreClient.tsx and /api/packs remain byte-for-byte unchanged, including pack
 * odds/RNG, operation idempotency, inventory consumption, duplicate dust and
 * cosmetic minting. Full CI plus real browser pack-opening screenshot 35 and
 * manifest evidence are mandatory before promotion.
 *
 * Deck Builder premium break-glass 2026-09-14: layout.tsx is intentionally
 * recertified only to load Visual 5.8 after Visual 5.7. Visual 5.8 styles the
 * existing Forge summary, editor, regional identity, private deck library,
 * format catalog, composition tray, diagnostics and synergy surfaces. ForgeClient
...
assert.ok(layout.indexOf(playerJourneyLayer) > layout.indexOf(metagamePremiumLayer), "Visual 5.2 player journey premium must load after Visual 5.1 metagame premium");
assert.ok(layout.indexOf(pvpLobbyLayer) > layout.indexOf(playerJourneyLayer), "Visual 5.3 PvP lobby premium must load after Visual 5.2 player journey premium");
assert.ok(layout.indexOf(rankedCompetitiveLayer) > layout.indexOf(pvpLobbyLayer), "Visual 5.4 Ranked competitive premium must load after Visual 5.3 PvP lobby premium");
assert.ok(layout.indexOf(draftPremiumLayer) > layout.indexOf(rankedCompetitiveLayer), "Visual 5.5 Draft premium must load after Visual 5.4 Ranked competitive premium");
