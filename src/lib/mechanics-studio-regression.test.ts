...
check("Mechanics editor locks identity keys after creation", mechanicsUi.includes("identityLocked = editingId !== null") && mechanicsUi.includes("readOnly={locked}") && mechanicsUi.includes("Identity key imutável") && mechanicsUi.includes("if (identityLocked) delete payload.key"));
