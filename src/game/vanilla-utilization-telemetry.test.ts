...
const matchup = vanillaBalanceMatchups()[0];
const seed = vanillaBalanceSeed(matchup, 0);
...
assert.deepEqual(
  ...
  plain,
  "opt-in utilization telemetry must not alter deterministic match results",
);
...
console.log(
  "VANILLA 1.2 UTILIZATION TELEMETRY: PASS — opt-in telemetry preserves deterministic summaries and records deck/card/type/policy utilization",
);
