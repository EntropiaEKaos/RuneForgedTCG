...
export function playFxGpuOverlay(target: Element | null, event: GameEvent, plan: FxExecutionPlan, quality: FxQuality): FxGpuHandle | null {
  if (!target || QUALITY_POINTS[quality] === 0 || typeof document === "undefined" || typeof window === "undefined") return null;
  const budget = buildFxVisualBudget(plan, quality);
  const count = budget.particles <= 0 ? 0 : Math.min(QUALITY_POINTS[quality], Math.max(1, budget.particles));
  ...
  try { gl = canvas.getContext("webgl", { alpha: true, antialias: false, depth: false, stencil: false, preserveDrawingBuffer: false }); } catch { return null; }
  if (!gl) return null;
  ...
  try {
    ...
    if (!gl.getShaderParameter(vertex, gl.COMPILE_STATUS) || !gl.getShaderParameter(fragment, gl.COMPILE_STATUS)) { cleanupGpu(); return null; }
    gl.attachShader(program, vertex);
    gl.attachShader(program, fragment);
    gl.linkProgram(program);
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) { cleanupGpu(); return null; }
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const ring = 0.28 + ((i * 17) % 9) / 18;
      points[i * 2] = Math.cos(angle) * ring;
      points[i * 2 + 1] = Math.sin(angle) * ring;
    }
    ...
    const c = gl.getUniformLocation(program, "c");
    const a = gl.getUniformLocation(program, "a");
    const color = COLORS[profile.identity] ?? COLORS.neutral;
    gl.uniform3f(c, color[0], color[1], color[2]);
    ...
    const draw = (now: number) => {
      ...
      gl!.uniform1f(s, progress);
      gl!.uniform1f(a, Math.sin(progress * Math.PI) * 0.72);
