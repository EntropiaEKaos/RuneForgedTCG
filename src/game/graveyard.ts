...
export function putInGraveyard(
  ...
  // Equipment slots currently use the same `eq_*` runtime representation for
  // a physical Equipment played from hand and an attachment generated by an
  // effect. Until the slot records physical-card provenance, adding either one
  // here could manufacture a graveyard card from a generated effect. Fail
  // closed for both in Zone 1.0; physical Equipment provenance is a follow-up.
  if (reason === "destroy" && sourceInstanceId?.startsWith("eq_")) {
    state.log.push(`${getCard(defId).name} leaves its bearer without a graveyard entry (equipment provenance unavailable).`);
    return null;
  }
