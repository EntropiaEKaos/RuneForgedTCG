    ...
    <div className="grid gap-5 p-5 xl:grid-cols-[1.05fr_1.4fr]">
      <div className="space-y-4">
        <div className="border border-white/8 bg-black/20 p-4">
          <div className="text-[8px] font-black uppercase tracking-[.18em] text-slate-600">Maior vazamento acumulado</div>
      ...
      <div>
        <div className="grid gap-2">
          {intelligence.trends24h.map(trend => <div key={trend.id} className="grid gap-2 border border-white/8 bg-black/20 px-4 py-3 sm:grid-cols-[1fr_auto_auto_auto] sm:items-center"><span className="text-[9px] font-bold uppercase tracking-[.12em] text-slate-500">{trend.label}</span><span className="text-xs font-bold text-slate-200">{trendValue(trend, trend.current)}</span><span className="text-[9px] text-slate-600">antes {trendValue(trend, trend.previous)}</span><span className={`text-[9px] font-black uppercase tracking-[.1em] ${trend.direction === "neutral" || trend.direction === "flat" ? "text-slate-500" : trend.direction === "up" ? "text-emerald-300" : "text-amber-200"}`}>{trend.direction === "up" ? "↑ " : trend.direction === "down" ? "↓ " : trend.direction === "flat" ? "→ " : "· "}{deltaLabel(trend.delta)}</span></div>)}
