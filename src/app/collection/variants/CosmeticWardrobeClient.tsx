  ...
  return (
    <main className="rf-app-page">
      ...
      <div className="rf-app-shell">
        ...
        <div className="mb-5 rounded-2xl border border-emerald-400/15 bg-emerald-400/[.05] p-4 text-sm text-slate-300"><b className="text-emerald-300">100% cosmético.</b> Variantes não alteram stats, efeitos, raridade de gameplay, limite de cópias, matchmaking ou regras competitivas.</div>
        ...
        <div className="mb-5 rounded-2xl border border-white/10 bg-slate-950/45 p-4"><input className="input w-full" placeholder="Buscar carta, variante, finish ou edição…" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
        ...
        {loading ? <Empty title="Sincronizando Ateliê" text="Carregando suas cópias colecionáveis individuais." /> : special.length === 0 ? <Empty title="Nenhuma variante especial encontrada" text="Quando uma cópia Full Art, Foil, Premium, Animated ou Serialized entrar no seu inventário ela aparecerá aqui. Suas cartas Standard continuam intactas." /> : (
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {special.map((asset) => {
              ...
              return (
                <article key={asset.assetId} className={`overflow-hidden rounded-3xl border bg-slate-950/60 shadow-xl ${equipped ? "border-amber-300/65 ring-2 ring-amber-300/20" : "border-white/10"}`}>
                  <div className="relative aspect-[16/10] bg-slate-900" style={artStyle}>
                    ...
                    {asset.serialNumber && <span className="absolute right-3 top-3 rounded-full border border-amber-200/50 bg-black/75 px-2 py-1 text-xs font-black text-amber-200">#{asset.serialNumber}{cosmetic.serialLimit ? `/${cosmetic.serialLimit}` : ""}</span>}
...
function Stat({ label, value, detail }: { label: string; value: number; detail: string }) { return <div className="rounded-2xl border border-white/10 bg-slate-950/45 p-4"><div className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">{label}</div><div className="mt-1 text-3xl font-black text-white">{value}</div><div className="mt-1 text-xs text-slate-400">{detail}</div></div>; }
