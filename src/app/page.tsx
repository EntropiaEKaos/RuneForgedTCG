...
import PlayerHeadquarters from "@/components/PlayerHeadquarters";
...
function safeInternalHref(value: string) { return value.startsWith("/") && !value.startsWith("//") ? value : ""; }
function compact(item: Awaited<ReturnType<typeof listPublishedSiteContent>>[number]): PublishedCard {
  return {
    ...
    ctaHref: safeInternalHref(text(item.payload.ctaHref) || text(item.payload.href)),
    ctaLabel: text(item.payload.ctaLabel) || "ABRIR EVENTO",
    startsAt: text(item.payload.startsAt) || text(item.payload.startAt),
    endsAt: text(item.payload.endsAt) || text(item.payload.endAt),
  };
}
function period(card: PublishedCard) {
  const format = (value: string) => {
    if (!value) return "";
    const date = new Date(value);
    return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString("pt-BR", { day: "2-digit", month: "short" });
  };
  const start = format(card.startsAt);
  const end = format(card.endsAt);
...
export default async function HomePage() {
  ...
  return (
    <main className="relative min-h-[calc(100vh-var(--forged-client-topbar))] overflow-hidden bg-[#05080d] text-slate-100">
      ...
      <Image src="/art/brand/runeforge-nexus-hero.webp" alt="" fill priority sizes="100vw" className="pointer-events-none object-cover opacity-35" />
        ...
        <div className="grid min-h-0 gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <section className="relative flex min-h-[520px] flex-col justify-end overflow-hidden border border-amber-200/10 bg-black/15 p-6 sm:p-8 lg:p-10">
            <div className="absolute inset-x-0 top-0 flex items-center justify-between border-b border-white/6 bg-black/20 px-5 py-3 text-[8px] font-bold uppercase tracking-[.22em] text-slate-500">
              <span>HOME · PLAYER EXPERIENCE 1.1</span>
            ...
            <div className="max-w-4xl pb-3">
              ...
              <h1 className="mt-4 max-w-3xl font-[var(--font-display)] text-4xl font-black leading-[1.05] tracking-[.035em] text-[#f4e7c7] sm:text-5xl lg:text-6xl">SUA FORJA.<br /><span className="text-amber-200/80">SEU LEGADO.</span></h1>
              <p className="mt-5 max-w-2xl text-sm leading-7 text-slate-300/72">Continue exatamente de onde parou: progressão, coleção, Ranked, eventos e mundo de FORGED convergem no mesmo client.</p>

              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Link href="/play" className="group inline-flex min-w-[220px] items-center justify-between border border-amber-100/35 bg-[linear-gradient(180deg,rgba(198,157,77,.22),rgba(118,86,31,.18))] px-6 py-4 font-[var(--font-display)] text-sm font-black tracking-[.16em] text-[#fff0c5] shadow-[0_15px_36px_rgba(0,0,0,.25)] transition hover:-translate-y-0.5 hover:border-amber-100/55 hover:bg-amber-100/15"><span className="flex items-center gap-3"><i className="not-italic">⚔</i> JOGAR</span><b className="transition group-hover:translate-x-1">→</b></Link>
                <Link href="/ranked" className="border border-white/12 bg-black/25 px-5 py-4 text-[10px] font-bold uppercase tracking-[.18em] text-slate-300 transition hover:-translate-y-0.5 hover:border-white/20 hover:text-white">FILA RANKED</Link>
                <Link href="/profile" className="border border-white/10 bg-black/20 px-5 py-4 text-[10px] font-bold uppercase tracking-[.18em] text-slate-400 transition hover:border-amber-100/20 hover:text-amber-100">MEU PERFIL</Link>
              </div>
            </div>
          </section>
          ...
          <aside className="grid min-h-0 content-start gap-4 overflow-y-auto pr-1">
            ...
            <section className="border border-white/8 bg-[#080d14]/90 p-4">
              ...
              {latestLore ? <Link href={`/lore/${encodeURIComponent(latestLore.slug)}`} className="mt-4 block border-l border-amber-200/22 pl-4 transition hover:border-amber-100/50">
            ...
            <section className="border border-sky-200/10 bg-[linear-gradient(135deg,rgba(56,189,248,.04),rgba(8,13,20,.94))] p-4">
              ...
              {latestEvent ? <div className="mt-4">
                <div className="flex items-center justify-between gap-3"><span className="text-[8px] uppercase tracking-[.18em] text-sky-200/45">LIVE OPS</span>{period(latestEvent) && <span className="text-[8px] uppercase tracking-[.12em] text-slate-600">{period(latestEvent)}</span>}</div>
                <h2 className="mt-2 font-[var(--font-display)] text-base font-bold text-slate-200">{latestEvent.title}</h2>
                <p className="mt-2 line-clamp-3 text-xs leading-5 text-slate-500">{latestEvent.summary}</p>
                {latestEvent.ctaHref && <Link href={latestEvent.ctaHref} className="mt-4 inline-flex border border-sky-200/15 bg-sky-200/[.04] px-3 py-2 text-[8px] font-black uppercase tracking-[.14em] text-sky-100">{latestEvent.ctaLabel} →</Link>}
              </div> : <p className="mt-4 text-xs leading-5 text-slate-600">Sem evento público publicado no momento.</p>}
            </section>
          </aside>
        </div>

        <section className="grid gap-2 border-t border-white/8 pt-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {QUICK.map((item) => <Link key={item.href} href={item.href} className="group flex min-h-[78px] items-center gap-3 border border-white/8 bg-black/25 px-4 py-3 transition hover:-translate-y-0.5 hover:border-amber-200/18 hover:bg-white/[.035]"><i className="w-7 text-center text-lg not-italic text-amber-200/55 transition group-hover:text-amber-100">{item.icon}</i><span className="min-w-0"><b className="block text-[10px] font-bold tracking-[.14em] text-slate-200">{item.label}</b><small className="mt-1 block truncate text-[9px] text-slate-600">{item.sub}</small></span></Link>)}
        </section>
      </div>
    </main>
  );
