...
export async function GET(req: NextRequest) {
  ...
  const [playersResult, activityResult, gameplayResult, economyResult, commerceResult, contentResult, journeyResult, marketplaceResult, retentionResult, topEventsResult, topRoutesResult, recentResult, pulseResult] = await Promise.all([
    ...
    db.execute(sql`select
      ...
      (select count(*)::int from (select player_id from matches where player_id is not null group by player_id having count(*) >= 2) second_matches) as second_match,
      (select count(*)::int from players where ranked_wins + ranked_losses > 0) as ranked_started,
      (select count(distinct player_id)::int from draft_sessions where player_id is not null) as draft_started,
      (select count(distinct player_id)::int from (
        select seller_player_id as player_id from market_listings
        union select buyer_player_id from market_listings where buyer_player_id is not null
        union select proposer_player_id from trade_offers
        union select recipient_player_id from trade_offers
      ) market_users) as marketplace_used,
      (select count(distinct player_id)::int from (
        select proposer_player_id as player_id from trade_offers where status='accepted'
        union select recipient_player_id from trade_offers where status='accepted'
      ) trade_users) as trade_accepted,
      (select count(*)::int from players p where exists (
        select 1 from telemetry_events t where t.player_id=p.id and t.created_at >= p.created_at + interval '7 days'
      )) as retained_7d`),
    db.execute(sql`select
      (select count(*)::int from market_listings where status='active' and expires_at > now()) as active_listings,
      (select count(*)::int from market_listings where listed_at >= ${dayAgo}) as listings_24h,
      (select count(*)::int from market_listings where completed_at >= ${dayAgo} and status='sold') as sold_24h,
      (select count(*)::int from trade_offers where status='active' and expires_at > now()) as active_trades,
      (select count(*)::int from trade_offers where created_at >= ${dayAgo}) as trades_24h,
      (select count(*)::int from trade_offers where completed_at >= ${dayAgo} and status='accepted') as accepted_trades_24h,
      (select count(*)::int from trade_offers where completed_at >= ${dayAgo} and status='expired') as expired_trades_24h`),
    db.execute(sql`select
      count(*) filter (where p.created_at <= ${weekAgo})::int as eligible_7d,
      count(*) filter (where p.created_at <= ${weekAgo} and exists (
        select 1 from telemetry_events t where t.player_id=p.id and t.created_at >= p.created_at + interval '7 days'
      ))::int as retained_7d,
      count(*) filter (where p.created_at <= ${monthAgo})::int as eligible_30d,
      count(*) filter (where p.created_at <= ${monthAgo} and exists (
        select 1 from telemetry_events t where t.player_id=p.id and t.created_at >= p.created_at + interval '30 days'
      ))::int as retained_30d
      from players p`),
  ...
  const players = rowsOf(playersResult)[0], activity = rowsOf(activityResult)[0], gameplay = rowsOf(gameplayResult)[0], economy = rowsOf(economyResult)[0], commerce = rowsOf(commerceResult)[0], content = rowsOf(contentResult)[0], journeyRow = rowsOf(journeyResult)[0], marketplace = rowsOf(marketplaceResult)[0], retention = rowsOf(retentionResult)[0], pulse = rowsOf(pulseResult)[0];
  const journey: CommandCenterJourney = {
    accountCreated:num(journeyRow,"account_created"), packOpened:num(journeyRow,"pack_opened"), deckCreated:num(journeyRow,"deck_created"),
    matchPlayed:num(journeyRow,"match_played"), matchWon:num(journeyRow,"match_won"), secondMatch:num(journeyRow,"second_match"),
    rankedStarted:num(journeyRow,"ranked_started"), draftStarted:num(journeyRow,"draft_started"), marketplaceUsed:num(journeyRow,"marketplace_used"),
    tradeAccepted:num(journeyRow,"trade_accepted"), retained7d:num(journeyRow,"retained_7d")
  };
  const current24h: CommandCenterWindow = { events:num(pulse,"current_events"), sessions:num(pulse,"current_sessions"), pvpCreated:num(pulse,"current_pvp_created"), pvpFinished:num(pulse,"current_pvp_finished"), orders:num(pulse,"current_orders"), approved:num(pulse,"current_approved"), revenueCents:num(pulse,"current_revenue") };
  const previous24h: CommandCenterWindow = { events:num(pulse,"previous_events"), sessions:num(pulse,"previous_sessions"), pvpCreated:num(pulse,"previous_pvp_created"), pvpFinished:num(pulse,"previous_pvp_finished"), orders:num(pulse,"previous_orders"), approved:num(pulse,"previous_approved"), revenueCents:num(pulse,"previous_revenue") };
  const intelligence = commandCenterIntelligence({
    journey, dau:num(activity,"dau"), wau:num(activity,"wau"), mau:num(activity,"mau"),
    pvpCreated24h:num(gameplay,"pvp_created_24h"), pvpFinished24h:num(gameplay,"pvp_finished_24h"),
    orders24h:num(commerce,"orders_24h"), approved24h:num(commerce,"approved_24h"),
    tradeCreated24h:num(marketplace,"trades_24h"), tradeAccepted24h:num(marketplace,"accepted_trades_24h"),
    eligibleRetention7d:num(retention,"eligible_7d"), retained7d:num(retention,"retained_7d"),
  ...
  return Response.json({ ok:true, generatedAt:new Date().toISOString(), windows:{ dayAgo:dayAgo.toISOString(), twoDaysAgo:twoDaysAgo.toISOString(), weekAgo:weekAgo.toISOString(), monthAgo:monthAgo.toISOString() },
    ...
    content:{ loreTotal:num(content,"lore_total"), lorePublished:num(content,"lore_published"), loreDrafts:num(content,"lore_drafts"), eventsPublished:num(content,"events_published"), newsPublished:num(content,"news_published") },
    marketplace:{ activeListings:num(marketplace,"active_listings"), listings24h:num(marketplace,"listings_24h"), sold24h:num(marketplace,"sold_24h"), activeTrades:num(marketplace,"active_trades"), trades24h:num(marketplace,"trades_24h"), acceptedTrades24h:num(marketplace,"accepted_trades_24h"), expiredTrades24h:num(marketplace,"expired_trades_24h") },
    retention:{ eligible7d:num(retention,"eligible_7d"), retained7d:num(retention,"retained_7d"), rate7d:num(retention,"eligible_7d") ? num(retention,"retained_7d") / num(retention,"eligible_7d") * 100 : 0, eligible30d:num(retention,"eligible_30d"), retained30d:num(retention,"retained_30d"), rate30d:num(retention,"eligible_30d") ? num(retention,"retained_30d") / num(retention,"eligible_30d") * 100 : 0 },
