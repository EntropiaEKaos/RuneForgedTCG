...
export async function GET(req: NextRequest) {
  ...
  const [playersResult, activityResult, gameplayResult, economyResult, commerceResult, contentResult, journeyResult, topEventsResult, topRoutesResult, recentResult, pulseResult] = await Promise.all([
  ...
  const players = rowsOf(playersResult)[0], activity = rowsOf(activityResult)[0], gameplay = rowsOf(gameplayResult)[0], economy = rowsOf(economyResult)[0], commerce = rowsOf(commerceResult)[0], content = rowsOf(contentResult)[0], journeyRow = rowsOf(journeyResult)[0], pulse = rowsOf(pulseResult)[0];
  ...
  const intelligence = commandCenterIntelligence({ journey, dau:num(activity,"dau"), wau:num(activity,"wau"), mau:num(activity,"mau"), pvpCreated24h:num(gameplay,"pvp_created_24h"), pvpFinished24h:num(gameplay,"pvp_finished_24h"), orders24h:num(commerce,"orders_24h"), approved24h:num(commerce,"approved_24h"), current24h, previous24h });
