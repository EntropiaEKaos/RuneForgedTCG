...
export async function POST(req: NextRequest, { params }: { params: Promise<{ resource: string; slug: string }> }) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [archived] = await tx.update(siteContent).set({
      status: "archived",
    ...
    await tx.insert(siteContentVersions).values({
      ...
      status: "archived",
      snapshot: { payload: archived.payload, seo: archived.seo },
      actor: actor.actorId,
      changeNote: sanitizeSiteChangeNote(body.changeNote, "Archived"),
    });
    await tx.insert(adminAuditLogs).values({
      action: "site_content.archive",
      resource: `site:${raw.resource}`,
      resourceId: archived.id,
      actor: actor.actorId,
      details: { slug, locale, version: archived.version, expectedVersion },
    });
