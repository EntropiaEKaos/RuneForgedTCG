...
export async function POST(req:NextRequest){
  try{
    ...
    const body=await req.json();const eventName=String(body.eventName||"").trim().slice(0,100);if(!/^[a-z0-9_.:-]{2,100}$/i.test(eventName))return Response.json({ok:false,error:"Invalid event"},{status:400});
  ...
  }catch{return Response.json({ok:false,error:"Invalid telemetry payload"},{status:400});}
}
