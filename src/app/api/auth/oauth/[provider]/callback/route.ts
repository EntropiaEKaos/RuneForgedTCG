...
export async function GET(req: NextRequest, ctx: { params: Promise<{ provider: string }> }) {
  ...
  if (req.nextUrl.searchParams.get("error")) return redirectError(req, "provider_cancelled");
  if (!tx || tx.provider !== provider || tx.state !== state || tx.exp < Date.now() || !code) return redirectError(req, "invalid_oauth_state");

  const setting = await getAuthProviderSetting(provider);
  if (!setting || !providerReady(setting)) return redirectError(req, "provider_unavailable");
  try {
    ...
    if (!tokenResponse.ok || !tokenPayload.access_token) return redirectError(req, "token_exchange_failed");
    ...
    if (!profileResponse.ok) return redirectError(req, "profile_fetch_failed");
    ...
    if (!subject) return redirectError(req, "profile_subject_missing");
    ...
    if (error instanceof IdentityConflictError) return redirectError(req, "identity_conflict");
