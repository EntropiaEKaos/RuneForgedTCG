...
The current Commander 4P four-browser-certification branch is based on post-merge-certified production `main` `f88e6ed3a9ff329888ef21445a832d91448f0825`. This slice adds browser-level proof only: the authoritative PostgreSQL room, combat envelope version 9, gameplay rules, reconnect contract and binary 1v1 engine remain unchanged. The cert uses four isolated Chrome profiles and four stable player sessions, seeds only CI-owned legal collections, then drives the public Commander UI and API boundary to prove 4/4 readiness, private projections, clockwise priority and revision convergence. Each new Commander HEAD is recertified independently; green results from an older SHA are never reused.